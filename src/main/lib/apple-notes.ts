...
export class AppleNotesExtractor {
  ...
  async init(folder: string, account: string): Promise<void> {
    ...
    this.folder = (
      await this.database
        .all`
        SELECT 
            Z_PK as z_pk, ZTITLE2 as ztitle2 
  ...
  async extractNoteHTML(name: string): Promise<string | void> {
    ...
    // if no notes are found, return null and handle it in the main process as a failure and keep going
    if (!notes) {
      console.error('MAIN: Note not found: ', name)
      return
    }
    console.log('MAIN: Note ', notes)
